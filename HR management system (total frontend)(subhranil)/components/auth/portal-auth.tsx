...
export function PortalAuth({
  ...
  function handleSubmit(event: React.FormEvent) {
    ...
    setTimeout(() => {
      ...
      if (valid) {
        // Log in the user with role-based data
        const role = theme.variant === "admin" ? "admin" : "employee"
        const userName = theme.variant === "admin" ? "Sarah Mitchell" : "John Anderson"
        
        login({
          id: theme.variant === "admin" ? "ADMIN001" : "EMP001",
          name: userName,
          email: email.trim().toLowerCase(),
          role: role,
          avatar: theme.variant === "admin" ? "S" : "J",
        })
